...
- **What the user sees:** A region picker. We attempt to **detect** the tenant's existing region first; otherwise the customer chooses from a publisher-maintained list of selectable regions (with a static fallback list if detection is unavailable).
